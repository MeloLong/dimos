#!/usr/bin/env python3
"""Deterministic offline benchmark for constrained M20 path smoothing."""
...
def _run_case(
    ...
    config = ConstrainedPathSmoothingConfig(
        validator_shadow_enabled=shadows_enabled,
        physical_validator_shadow_enabled=shadows_enabled,
    )
    runs: list[dict[str, float | int]] = []
    wall_times: list[float] = []
    cpu_times: list[float] = []
    output_points: set[int] = set()
    timing_supported = "timing" in inspect.signature(constrained_smooth_resample_path).parameters
            ...
            writer.writerow(
                {
                    ...
                    "shadows_enabled": case["shadows_enabled"],
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=("both", "shadow-on", "shadow-off"), default="both")
    ...
    cases = [
        _run_case(length, shadows, args.warmups, args.repetitions, args.seed)
        for shadows in modes
    ...
    result = {
        "protocol": {
            ...
            "timing_supported": "timing"
            in inspect.signature(constrained_smooth_resample_path).parameters,
            "implementation": inspect.getsourcefile(constrained_smooth_resample_path),
        },
        "cases": cases,
    }
