...
| 渲染与合成传感器 | 通过 | 前视 RGB、前后两台 96 通道 360 x 90 度半球射线雷达、非空点云及 `(0, 1)` 几何组下无机器人自扫描 |
...
128 个方位采样、96 个垂直通道、360 x 90 度半球视场、0.1 m 最小距离和 10 m 运行截断。实物 Airy
驱动支持 60 m，点流也远比仿真稠密；这里降低方位密度与发布频率是为了保持导航仿真实时性。当前尚未
复现出厂非均匀束角、扫描时序、噪声、运动畸变、遮挡和 M20 精确安装外参。移动 mocap 人物对传感器
可见，但默认不会与 M20 发生物理碰撞，因为 mocap 物体质量等效为无穷大。
