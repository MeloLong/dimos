...
RGB 与合成深度相机是 DimOS 为仿真添加的内容。官方 SDK 资产没有给出实体 M20 相机/雷达的具体
型号、内参、扫描线分布、时序或标定，因此这些数据流不能视为与实车传感器等价。
...
| 渲染与合成传感器 | 通过 | 前视 RGB、前后 48 行合成深度、非空点云及 `(0, 1)` 几何组下无机器人自扫描 |
...
默认配置只发布 640 x 360、10 Hz 的前视 RGB，以及 2 Hz 的前/后合并深度点云。每个合成深度视场为
640 个水平采样、48 个垂直采样行，最远 10 m。48 行用于近似降低后的垂直雷达通道数，不等价于经过
标定的实体 48 线雷达扫描模型。移动 mocap 人物对相机可见，但默认不会与 M20 发生物理碰撞，因为
mocap 物体质量等效为无穷大。
