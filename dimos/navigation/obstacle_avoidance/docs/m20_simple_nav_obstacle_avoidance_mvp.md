...
**版本：** v1.7（new_simple_nav_merged_main 迁移版）
**日期：** 2026-07-23
**开发目录：** `/home/markus/work/dimos_m20`
**开发分支：** `wd/m20-mujoco-simulation@42e83165`
**当前迁移工作区：** `/home/markus/work/dimos_obstacle_avoidance_new_simple_nav`
**迁移目标：** `melolong/new_simple_nav_merged_main@7206ac17`
...
最初的 MuJoCo 验证版只接入 `m20_simple_nav_sim`。后续分阶段迁移已把模块接入
真实 M20 的 `m20_simple_nav` 蓝图，但实车配置保持 `enabled: false`，避免在
完成地图时效和制动参数标定前启用仿真来源的安全参数。
...
- 原始仿真阶段只验证 `m20_simple_nav_sim`；当前迁移同时验证真实 M20 蓝图中
  模块默认关闭且接线唯一。
- 关闭模块时仿真命令逐字段透传。
...
- 原始仿真实现阶段未修改真实 M20 蓝图；后续迁移已接入模块，但实车配置默认
  关闭，RayTracer 仍使用 `global_emit_every: 10`。
...
## 18. new_simple_nav_merged_main 迁移状态

ObstacleAvoidance 首先通过 PR #8 合并到 `wd/dimos/test`。本阶段不合并整个
`wd/dimos/test`，而是从当前 `new_simple_nav_merged_main@7206ac17` 创建独立
迁移分支，只同步停障模块及其直接依赖：

```text
branch: codex/obstacle-avoidance-new-simple-nav
worktree: /home/markus/work/dimos_obstacle_avoidance_new_simple_nav
```
...
- 目标分支现有规划、平滑、回放和新 blueprint namespace 行为保持不变。
- 生成模块注册表已加入 `obstacle-avoidance`。
- M20 专项测试使用目标分支的字符串 namespace 检查 remapping。
...
这组参数等价于迁移前固定 3 m 检查。待实车验证后，才考虑切换为
...
6. 确认关闭配置时与原 `new_simple_nav_merged_main` 行为一致，再单独打开停障
   模块。
...
target module/config/blueprint tests: 36 passed
expanded non-LFS target regression: 51 passed
```

这些结果不包括物理 M20 运行、真实障碍停车、真实 costmap 频率或制动距离验证。
迁移分支必须在目标回归通过后提交并通过 PR 合并回
`new_simple_nav_merged_main`；实车配置在完成前置验证前继续保持关闭。
